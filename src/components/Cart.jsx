...
export default function Cart() {
  ...
  const cards = products.map((product, idx) => (
    <div key={idx} className="my-2 d-flex justify-content-center">
       <Card style={{ width: "18rem" }}className='d-flex justify-content-center'>
        ...
        <Card.Body style={{height: "120px"}} className='d-flex justify-content-center'>
        ...
        <Card.Footer className='d-flex justify-content-center '>
          <Button variant="danger" onClick={()=> removeToCart(product._id)}>Remove</Button>
        </Card.Footer>
      </Card>
    </div>
  ));
  ...
  return (
    <>
      <Row>
        <Col >
        {cards.length ? cards : <div style={{height:"500px"}} className='d-flex flex-column justify-content-center text-center'><h1>Cart is empty</h1></div>}
        </Col>
        <Col xs={4}>
          <Card className='mt-4'>
